...
#region Events
        ...
        void ProbeGround(float dt)
        {
            ...
            if (slopeAngle <= slopeLimit && EvaluateGroundStability(collisionInfo.hitInfo.transform))
            {
                // Stable hit ---------------------------------------------------
                SetGroundInfo(in collisionInfo, true);
            ...
            while (iteration < CharacterConstants.MaxSlideIterations || displacement == Vector3.zero)
            {
                ...
                if (collisionInfo.hitInfo.hit)
                {
                    ...
                    if (CheckOneWayPlatformLayerMask(collisionInfo))
                    {
                        ...
                        // Check if the character is going to hit the platform with the bottom part of the capsule.
